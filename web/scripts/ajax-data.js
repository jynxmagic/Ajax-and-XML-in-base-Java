...
function showXmlCityInfo(request, resultRegion) {
  if ((request.readyState == 4) &&
    ...
    var cities = xmlDocument.getElementsByTagName("customer");
    var rows = new Array(cities.length);
    var subElementNames = ["firstName", "lastName", "balance"];
...
function getTwoCustomersTable(formatField, resultRegion) {
  var address = "customer";
  var responseHandler = findHandler("xml");
  ajaxPost(address, null,
      function(request) {
        document.getElementById(resultRegion).innerText = request.response;
      });
}
